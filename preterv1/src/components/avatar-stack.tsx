...
function AvatarContent({ user, labelStyle }: { user: RoomUser; labelStyle: object }) {
  if (user.avatarUrl) {
    return <Image source={{ uri: user.avatarUrl }} style={StyleSheet.absoluteFill} resizeMode="cover" />;
  }
  return <Text style={labelStyle}>{initialOf(user.displayName)}</Text>;
}
  ...
  if (users.length <= 1) {
    return (
      <View style={styles.single}>
        {users[0] ? <AvatarContent user={users[0]} labelStyle={styles.label} /> : <Text style={styles.label}>1</Text>}
      </View>
    );
  }
